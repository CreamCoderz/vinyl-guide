...
describe EbayClient do
  ...
  it "should not exceed 20 items details per request" do
    ...
    response_data = ['', '']
    for i in (1..30)
      ebay_item = EbayItemData.new("description #{i}", i, Time.new, Time.new, "http://www.ebay.com/#{i}}", "http://img.com/#{i}", i, 10.0 + i, "steve#{i}", "title#{i}", ["http://example.com/#{i}", "http://example.com/#{i+1}"])
