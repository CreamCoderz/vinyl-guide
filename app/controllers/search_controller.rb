...
class SearchController < ApplicationController
  ...
  def search
    ...
    @page_results = EbayItem.search(:query => @query, :column => @sort_param,
                                                                      :order => @order_param, :page => page_num,
                                                                      :include_mapped => include_mapped)
    @sortable_base_url = "/search?q=#{@query}&page=#{page_num}"
    if request.xml_http_request?
     render :json => {:hits => 0, :ebay_items => JSON.parse(@page_results.items.to_json(:only => [:id, :title], :methods => [:link]))}
    end
  end
